...
public class BulkExhaustAction extends AbstractGameAction {
    private ArrayList<AbstractCard> targetCards;
    private CardGroup group;

    public BulkExhaustAction(ArrayList<AbstractCard> targetCards) {
        this.targetCards = targetCards;// 16
        this.actionType = ActionType.DISCARD;// 17
        this.duration = Settings.ACTION_DUR_FAST;// 18
    }// 19

    public BulkExhaustAction(ArrayList<AbstractCard> targetCards, CardGroup group) {
    ...
    public void update() {
        ...
        if(!targetCards.isEmpty()){
            ...
            if(this.group.contains(c)){
                this.group.moveToExhaustPile(c);// 35

                CardCrawlGame.dungeon.checkForPactAchievement();// 34
